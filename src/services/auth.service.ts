...
export class AuthService extends BaseService {
  ...
  private initializeQueue(): void {
    ...
    this.queueService.registerProcessor(
      ...
      async (job) => this.processRemoveRefreshToken(job)
    );
  ...
  public async refreshTokens(refreshToken: string) {
    const tokenRecord = await this.authRepository.findByRefreshToken(
      refreshToken
    );
    if (!tokenRecord) {
  ...
  public async logout(refreshToken: string): Promise<void> {
    const token = await this.authRepository.invalidateRefreshToken(
      refreshToken
    );
    if (!token) {
      throw new CustomError("Invalid token", HttpStatus.UNAUTHORIZED);
  ...
  public async storeRefreshToken(userId: number, token: string): Promise<void> {
    ...
    const tokenData = await this.authRepository.storeRefreshToken(
      userId,
      token,
      expiresAt
    );
